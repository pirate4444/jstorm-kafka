 ...
 *
 */
public class PartitionConsumer implements IPartitionConsumer{
	...
	public PartitionConsumer(Map conf, KafkaSpoutConfig config, int partition, ZkState offsetState) {
		...
		try {
			if (config.fromBeginning) {
				...
				if (jsonOffset == null) {
					lastCommittedOffset = consumer.getOffset(config.topic, partition,
	...
	private void fillMessages() {
		...
		try {
			...
			if (msgs == null) {
				...
				// 如果取到空，则改为从最后的Offset取数 （梁毅 2017-09-26）
				long latestOffset = consumer.getOffset(config.topic, partition,
						kafka.api.OffsetRequest.LatestTime());
			...
			for (MessageAndOffset msg : msgs) {
				...
				// LOG.debug("fillmessage fetched a message:{}, offset:{}",
				// msg.message().toString(), msg.offset());
			}
			// long end = System.currentTimeMillis();
			// LOG.info("fetch message from partition:"+partition+", offset:" +
			// emittingOffset+", size:"+msgs.sizeInBytes()+", count:"+count +",
			// time:"+(end-start));
		} catch (Exception e) {
			e.printStackTrace();
			LOG.error(e.getMessage(), e);
	...
	public void commitState() {
		try {
			...
			if (lastOffset != lastCommittedOffset) {
				...
				data.put("broker", ImmutableMap.of("host", consumer.getLeaderBroker().host(), "port",
						consumer.getLeaderBroker().port()));
